...
/// <summary>Command for <c>PATCH /shorfah/sections/{id}</c> (BUSINESS-RULES.md §1.4). Ports <c>shorfah.ts:344-397</c>.</summary>
...
public sealed record PatchShorfahSectionCommand(
    int ActorUserId,
    int SectionId,
    string? ContentMd,
    string? ContentHtml,
    ...
    DateTimeOffset? SlaDeadline) : IRequest<Result<ShorfahSectionDto>>;
