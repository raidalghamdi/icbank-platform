...
/// <summary>
/// In-memory <see cref="IDesignGenerationRateLimiter"/>, following the exact structure of
/// <c>InMemoryInternationalDaySearchRateLimiter</c> (wave 2) but keyed by user id. Registered as
/// a singleton so window state survives across requests within one process. Does not coordinate
/// across multiple instances -- same documented limitation as its wave-2 counterpart.
/// </summary>
public sealed class InMemoryDesignGenerationRateLimiter : IDesignGenerationRateLimiter
{
    ...
    /// <summary>Initializes a new instance of the <see cref="InMemoryDesignGenerationRateLimiter"/> class.</summary>
    /// <param name="dateTimeProvider">The injectable clock.</param>
    public InMemoryDesignGenerationRateLimiter(IDateTimeProvider dateTimeProvider)
    {
