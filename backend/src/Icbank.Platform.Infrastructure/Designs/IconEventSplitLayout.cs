...
internal static class IconEventSplitLayout
{
    internal static string Render(IconEventRenderContext context)
    {
        ...
        return $"<div class=\"poster split-layout\" style=\"{PosterStyle(context)}display:flex;\">{Chrome(context)}{RenderIconColumn(context, reserve)}{RenderTextColumn(context, reserve)}{Footer(context)}</div>";
    }
